...
ConnectionHandler::ConnectionHandler(Reactor* reactor) : _reactor(reactor), _processor(this)
{
}
...
void ConnectionHandler::prepareWrite(int fd, const std::string& buffer)
{
    ...
    // Modify the fd to be write events to notify the reactor to trigger the response is ready to send
    ev.events = EPOLLOUT;
    ev.data.fd = fd;
...
{
    // TODO: the CHUNKED_SIZE might be same as the max client request size
...
void ConnectionHandler::_handleWrite(int fd)
{
    ...
    if (it == _write_buffer.end())
    {
        weblog::logger.log(weblog::ERROR,
                           "No write buffer found for fd: " + utils::toString(fd));
        return;
    } else {
        weblog::logger.log(weblog::DEBUG,
        ...
        else if (bytes_sent == 0)
        {
            ...
            closeConnection(fd, weblog::INFO, "Write 0 bytes, client closing connection");
        }
        else
        {
            weblog::logger.log(weblog::DEBUG,
            ...
            // Modify the fd to be read events to notify the reactor to trigger the next request
            struct epoll_event ev;
            ev.events = EPOLLIN;
            ev.data.fd = fd;
            epoll_ctl(_reactor->epollFd(), EPOLL_CTL_MOD, fd, &ev);
